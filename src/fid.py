...
def save_model_samples(name_G, name_SDE, net_G, net_f, data_iterator, gamma):
    if os.path.exists(name_G):
        shutil.rmtree(name_G)
    os.makedirs(name_G, exist_ok=True)
        
    if os.path.exists(name_SDE):
        shutil.rmtree(name_SDE)
    os.makedirs(name_SDE, exist_ok=True)
    ...
    for batch in data_iterator:
        ...
        G_X = net_G(X)
        out_G = (G_X.clamp(-1,1) * 127.5 + 128).to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()
        if net_f is not None:
            SDE_X = net_f(X, gamma)
            out_SDE = (SDE_X.clamp(-1,1) * 127.5 + 128).to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()
        for i in range(out_G.shape[0]):
            img_G = Image.fromarray(out_G[i])
            if net_f is not None:
                img_SDE = Image.fromarray(out_SDE[i])
            n_digits = len(str(count))
            img_name = (6 - n_digits) * '0' + str(count) + '.png'
            img_G.save(os.path.join(name_G, img_name))
            if net_f is not None:
                img_SDE.save(os.path.join(name_SDE, img_name))
            count += 1
...
def calculate_inception_stats(
    ...
    for images, _labels in tqdm(data_loader, unit='batch'):
        if images.shape[0] == 0:
            continue
        if images.shape[1] == 1:
