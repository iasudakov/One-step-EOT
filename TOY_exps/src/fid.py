...
def save_model_samples(name, net, data_iterator, batch_size, num_samples, device, name2, data2_iterator):
    ...
    with tqdm(range(num_samples)):
        while count < num_samples:
            ...
            X = batch[0][:cur_batch_size].to(device)
            T_X = net(X)
    ...
    with tqdm(range(num_samples)):
        while count < num_samples:
            cur_batch_size = min(num_samples - count, batch_size)
            batch = next(iter(data2_iterator))
            X = batch[0][:cur_batch_size].to(device)
            out = (X.clamp(-1,1) * 127.5 + 128).to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()
            for i in range(out.shape[0]):
                img = Image.fromarray(out[i])
                n_digits = len(str(count))
                img_name = (6 - n_digits) * '0' + str(count) + '.png'
                img.save(os.path.join(name2, img_name))
                count += 1
            
    return l2_sum/count, lpips_sum/count
